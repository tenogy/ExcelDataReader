...
namespace ExcelDataReader.Portable
{
	...
	public class ExcelBinaryReader : IExcelDataReader
	{
		...
		public async Task<bool> Read()
		{
			...
			return await Task.FromResult(moveToNextRecord());
		}
        ...
        public async Task LoadDataSetAsync(IDatasetHelper datasetHelper, bool convertOADateTime)
        {
            ...
            await readAllSheets(datasetHelper);
	    ...
	    private async Task readAllSheets(IDatasetHelper datasetHelper)
	    {
	        for (int index = 0; index < ResultsCount; index++)
	        {
	            await readWholeWorkSheet(m_sheets[index], datasetHelper);
	        }
	    }

	    private async Task readWholeWorkSheet(XlsWorksheet sheet, IDatasetHelper datasetHelper)
        {
            XlsBiffIndex idx;
            ...
            if (idx != null)
                ...
                await readWholeWorkSheetNoIndex(triggerCreateColumns, datasetHelper);
        ...
        private async Task readWholeWorkSheetNoIndex(bool triggerCreateColumns, IDatasetHelper datasetHelper)
        {
            while (await Read())
            {
                if (m_depth == m_maxRow) break;
